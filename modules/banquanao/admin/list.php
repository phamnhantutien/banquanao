...
if ($nv_Request->isset_request("action", "post,get")) {
    ...
    if ($id > 0 and $checksess == md5($id . NV_CHECK_SESSION)) {
        $db->query("DELETE FROM `nv4_banquanao_user` WHERE id=" . $id);
    }
}
...
if (!empty($array_row)) {
    ...
    foreach ($array_row as $row) {
        ...
        $row['url_edit'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '='
            . $module_name . '&amp;' . NV_OP_VARIABLE . '=main&amp;id=' . $row['id'];
        $row['url_delete'] = NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '='
            . $module_name . '&amp;' . NV_OP_VARIABLE . '=list&amp;id=' . $row['id'] . '&action=delete&checksess=' . md5($row['id'] . NV_CHECK_SESSION);
        $xtpl->assign('ROW', $row);
